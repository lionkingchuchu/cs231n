...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        xqxk = torch.matmul(query, torch.swapaxes(key, 2, 3)) #N H S T
        xqxk /= torch.sqrt(torch.tensor(E/H))

        if attn_mask != None:
          xqxk = xqxk.masked_fill(attn_mask == 0, float('-inf'))
        
        out = self.attn_drop(torch.softmax(xqxk, dim = -1)) #N H S T
        out = torch.matmul(out, value) #N H S T x N H T E//H = N H S E//H
